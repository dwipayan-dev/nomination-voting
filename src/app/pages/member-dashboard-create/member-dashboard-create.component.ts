...
@Component({
  selector: 'app-member-dashboard-create',
  standalone: true,
  ...
  templateUrl: './member-dashboard-create.component.html',
  styleUrl: './member-dashboard-create.component.css'
})
export class MemberDashboardCreateComponent {
  hide = true;
  private CountryURL = url.app_url + "country";
  private CreateMember = url.app_url + "member/store";
  country: any = [];
  profile_pic!: File;
  append_profile_pic = "/assets/avatar_white.gif"
  constructor(private http: HttpClient, private handleError: CatchError, private el: ElementRef, private renderer: Renderer2, private router: Router, private _location: Location) {
  ...
  form: any = new FormGroup({
    ...
    profilePicFormControl: new FormControl('', [Validators.required]),
    nominationFormControl: new FormControl('yes', [Validators.required]),
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      this.http.post(this.CreateMember, formData).pipe(
        catchError(e => {
          return this.handleError.handleError(e);
        })).subscribe(((res: any) => {
